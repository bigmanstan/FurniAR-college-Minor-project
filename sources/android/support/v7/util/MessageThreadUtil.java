...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            class C02811 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem msg = C04901.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C02821();
            ...
            class C02821 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C04912.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C04912.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C04912.this.mQueue.removeMessages(2);
                                    C04912.this.mQueue.removeMessages(3);
                        ...
                        C04912.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
